...
func (this ChatService) GetManagerMsg(userid int) []map[string]interface{} {
	sql := `SELECT id, sender_uid, content, create_time FROM 
			(SELECT id, sender_uid, content, create_time 
			FROM tb_chat_manager WHERE sender_uid = ? OR recver_uid = ? ORDER BY id DESC LIMIT 15) AS tb_c
			ORDER BY tb_c.id ASC
		   `
